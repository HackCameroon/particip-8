...
public class HoundifyVoiceSearchWithPhraseSpotterActivity extends AppCompatActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        houndify.setVoiceSearchBuilderInterceptor(new Houndify.VoiceSearchBuilderInterceptor() {
            ...
            public void intercept(VoiceSearch.Builder builder) {
                ...
                // Input Language
                builder.setInputLanguageEnglishName("English");
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        ArrayList<String> timmyp = new ArrayList<String>();
        timmyp.add("The dog is red");
        timmyp.add("The apple was juicy");
        timmyp.add("two plus two is four");
        mmap.put("timmy", timmyp);

        ArrayList<String> lizzy = new ArrayList<String>();
        lizzy.add("I ate oatmeal for breakfast");
        lizzy.add("The main idea was don't steal");
        mmap.put("lizzy", lizzy);
    ...
    private void onResponse(final HoundResponse response) {
        ...
        String f = response.getDisambiguation()
                .getChoiceData()
                .get(0)
                .getTranscription().split(" ")[0];
String currname = f;
        String temp = "";
        f += " participated! They said:";
        for (int i = 1; i < response.getDisambiguation()
                .getChoiceData()
                .get(0)
                .getTranscription().split(" ").length; i++)
        {
            temp+= response.getDisambiguation()
                    .getChoiceData()
                    .get(0)
                    .getTranscription().split(" ")[i];
            temp+= " ";
        ...
        if(!mmap.containsKey(currname))
        {
            ArrayList<String> tset2 = new ArrayList<String>();
            mmap.put(currname,tset2);
        }
        ArrayList<String> tset = mmap.get(currname);
        tset.add(temp);


        mmap.put(currname,tset);
        f = f + temp;
        ...
        for (Map.Entry<String,ArrayList<String>> entry : mmap.entrySet()) {
            ...
            k += "Student: " + entry.getKey() + "\nParticipation : " + entry.getValue().get(i) + "\n";
        }
        k+= "\nAnalysis: \n";
        for (Map.Entry<String,ArrayList<String>> entry : mmap.entrySet())
        {
            k+= entry.getKey() + " participated " + entry.getValue().size()+ " times \n";
    }
        textView.setText(k);
